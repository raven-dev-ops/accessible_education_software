...
async function run() {
  ...
  for (const route of pages) {
    console.log(`A11y scan for ${route} (base: ${baseURL})`);
    await page.goto(route, { waitUntil: "networkidle" });
    const results = await new AxeBuilder({ page }).analyze();
    if (takeScreenshots) {
      const safe = route.replace(/[^a-z0-9]/gi, "_").replace(/_+/g, "_") || "home";
      await page.screenshot({ path: `${screenshotDir}/${safe}.png`, fullPage: true });
    }
    if (results.violations.length) {
      console.warn(`Found ${results.violations.length} violations on ${route}`);
